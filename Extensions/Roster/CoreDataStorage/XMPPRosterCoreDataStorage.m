...
- (BOOL)removeUserWith:(XMPPJID *)jid xmppStream:(XMPPStream *)stream
{
    XMPPLogTrace();
...
- (NSArray *)myFriendsForXMPPStream:(XMPPStream *)stream{
    ...
    [self executeBlock:^{
        ...
        if (stream)
        {
            ...
            NSSortDescriptor * sort = [[NSSortDescriptor alloc] initWithKey:@"displayName" ascending:YES];
            // NSSortDescriptor * sort2= [[NSSortDescriptor alloc] initWithKey:@"presenceStr" ascending:NO];
            [fetchRequest setSortDescriptors:[NSArray arrayWithObjects:sort,nil]];
            
            [fetchRequest setPredicate:predicate];
        }
        
        results = [NSMutableArray arrayWithArray:[moc executeFetchRequest:fetchRequest error:nil]];
    ...
    return [results sortedArrayUsingSelector:@selector(compareByAvailabilityName:)];
}
